...
export async function GET({ request }) {
    ...
    try {
        ...
        try {
            const data = await database.ref(`weeklyPlan/${uid}`).get();
            const weeklyPlan = data.val();
...
export async function POST({ request }) {
    ...
    if (!collectionId || !recipeId || !date) return new Response('400 Bad Request', { status: 400 });
...
export async function DELETE({ request, url }) {
    ...
    if (!index || !date) return new Response('400 Bad Request', { status: 400 });
